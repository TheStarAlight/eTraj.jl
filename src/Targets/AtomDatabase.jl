...
"Constructs a `HydrogenLikeAtom` or `SAEAtom` based on the `name` and `charge` given, other initialization parameters can be passed via `kwargs`."
function get_atom(name::String, charge::Integer=0; kwargs...)
    HydLikeAtomData = Dict{String, Any}(
        "H" => Dict{Symbol, Any}(
            :Ip => 0.5,
            :Z  => 1,
            :name => "H"
