...
struct GaussianLaser <: MonochromaticLaser
    ...
    """
    function GaussianLaser(peakInt, waveLen, spreadCycNum, ellip, cep=0., t_shift=0.)
        ...
        @assert 0≤ellip≤1   "[GaussianLaser] Ellpticity must be in [0,1]."
        new(peakInt,waveLen,spreadCycNum,ellip,cep,t_shift)
    end
    function GaussianLaser(;peakInt, waveLen, spreadCycNum, ellip, cep=0., t_shift=0.)
        GaussianLaser(peakInt,waveLen,spreadCycNum,ellip,cep,t_shift)
    end
    """
    Constructs a new monochromatic elliptically polarized laser field with Gaussian-shape envelope.
    # Parameters
...
Ellpticity(l::GaussianLaser) = l.ellip
...
function LaserAx(l::GaussianLaser)
    local A0 = LaserA0(l); local ω = AngFreq(l); local σ = SpreadDuration(l); local φ = l.cep; local Δt = l.t_shift;
    return function(t)
        t -= Δt
...
function LaserAy(l::GaussianLaser)
    local A0 = LaserA0(l); local ω = AngFreq(l); local σ = SpreadDuration(l); local φ = l.cep; local Δt = l.t_shift; local ε = l.ellip;
    return function(t)
        t -= Δt
